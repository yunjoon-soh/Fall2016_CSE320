...
int part4(size_t nthreads){
	...
	for(int i = 0; i < nthreads * BUF_SIZE; i++){
		buf[i] = (struct map_res *) malloc(sizeof(struct map_res));
		buf[i]->tot_duration = 0;
	}
	...
	sem_t buf_sem[nthreads * BUF_SIZE];
	
	Sem_init(&mutex, 0, 1); /*For locking the writecnt*/
	Sem_init(&r, 0, 1); /*For locking the writing action*/
	for(int i = 0; i < nthreads * BUF_SIZE; i++){
		Sem_init(&buf_sem[i], 0, 1);
	}
	Sem_init(&write_ind_sem, 0, 1);
	writecnt = 0;
...
static void* map(void* v){
	...
	for(int i = 0; i < per_thread; i++){
		...
		Write_struct_r(mr); // write to temp file only when possible
